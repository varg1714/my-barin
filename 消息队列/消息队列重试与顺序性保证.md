...
- 投递回原队列
	...
	`ImmediateRequeueMessageRecoverer` 的实现逻辑为直接抛出了一个异常，`AcknowledgeMode` 设置为"auto"，在成功时 ack 消息，在失败时 nack 消息；`AcknowledgeMode` 设置为"manual"，并且此时消息进入到了 `MessageRecoverer` 中，这就意味着**消费者代码包含未处理的异常且未进行消息确认**，因此也可直接抛出异常，这时 Rabbbit 消息未收到 ack 确认从而也会触发 requeue 操作。
- 自定义失败逻辑
...
![](https://r2.129870.xyz/img/20220728113547.svg)
