...
- 跨 Region 引用对象如何解决
    ...
    2. **记忆集 (RSet) 用于解决跨区域引用问题**
        ...
        **并发优化** (后台 Refinement 线程) ：G1 有专门的后台线程，称为并发优化线程（Concurrent Refinement Threads） 。这些线程会周期性地扫描全局卡表，寻找“脏卡”。当找到一个脏卡，线程会扫描这个卡片覆盖的 512 字节内存，找出其中所有指向外部 Region 的引用。于是，该线程会外部 Region A 的 RSet，在其中记下：“Region X 的第 N 号卡片引用了我”，更新完 RSet 后，这个脏卡状态会被清理，以便下次复用。
