...
可通过 `-XX:MetaspaceSize=64m -XX:MaxMetaspaceSize=128m` 限制方法区的大小，根据《Java 虚拟机规范》的规定，如果方法区无法满足新的内存分配需求时，将抛出 OutOfMemoryError 异常。
...
CMS 作为老年代收集器，采用卡表记录新生代的跨代引用，将老年代划分为 512 字节的卡表，当 YoungGC 进行时，仅需扫描这些涉及新生代引用的老年代即可。
...
2. 并发标记（CMS concurrent mark）
    ...
    由于在并发标记阶段和用户线程并发，因此可能产生新的对象或者对象引用关系发生变化：

    - 新生代对象晋升到老年代
    - 直接在老年代分配对象
    - 引用关系变化

    对于这些对象，需要重新标记。为了提高重新标记的效率，本阶段会把这些发生变化的对象所在的 Card，通过写屏障标识为 Dirty，这样后续就只需要扫描这些 Dirty Card 的对象，从而避免扫描整个老年代。

1. 重新标记（CMS remark）
    重新标记阶段则是为了修正并发标记期间，因用户程序继续运作而导致标记产生变动的那一部分对象的标记记录（CMS 基于增量更新来重新标记变动的记录），这个阶段的停顿时间通常会比初始标记阶段稍长一些，但也远比并发标记阶段的时间短。

    重新标记（Remark）阶段是一个 STW（Stop-The-World）阶段，它的目标是**最终确保所有存活对象都被正确标记，防止任何“漏标”**。为了达到这个目的，它需要做一个全面、无遗漏的最终检查。主要工作包括：

    1. 遍历新生代对象
        全面扫描新生代（Young Generation）中的所有对象，查找并标记所有从新生代指向老年代的存活对象。这是非常重要的一步，因为在并发标记期间，新生代可能发生过多次 GC，对象和引用关系变化非常频繁。
    2. 重新从 GC Roots 开始标记
        像初始标记一样，快速地再扫描一遍 GC Roots，以防在并发阶段有新的 GC Roots 产生或变化。
    3. 处理所有剩余的“脏卡”
        处理在并发标记、预清理、可终止的预清理阶段之后，仍然被标记为“脏”的卡片。这部分为“增量更新”所处理的情况。虽然预清理阶段已经处理了大部分，但总会有一些“漏网之鱼”，必须在 STW 阶段彻底清算。
...
在并发标记和重新标记之间，还存在并发预清理和可中断的并发预清理这两个阶段，这两个阶段的作用是尽可能的再次标记对象解决漏标问题，**减少重新标记阶段的工作**：

1. 预清理（PreClean）阶段
    **预清理阶段提前把并发标记期间产生的“已知”的引用变化给处理掉**，避免这些工作全部堆积到需要 STW 的“重新标记”阶段：
    
    - 扫描脏卡
        并发标记阶段对象引用关系可能变更，CMS 通过‌写屏障（Write Barrier） 将修改引用的对象所在内存区域标记为 “脏卡”‌。预清理阶段会扫描这些脏卡，重新标记受影响的存活对象，避免最终标记阶段（Final Remark）因处理大量脏卡而延长 STW 时间‌。

        例如当并发标记时，一个黑色对象 A（已标记完）突然引用了一个白色对象 C（未标记），`A.field = C`。如果不处理，C 就会被漏标。

    - 触发年轻代 GC
        若开启 ` ‌-XX:+CMSScavengeBeforeRemark`，预清理阶段可能触发年轻代 GC，将存活对象晋升至老年代，减少重新标记阶段的跨代引用复杂度‌。
2. 可终止预清理阶段（Abortable Preclean）
    - 扫描新生代对象，标记可达的老年代对象
        通过尽可能等待一次 Young GC 触发‌，触发年轻代 GC，减少存活对象晋升到老年代的概率，从而降低重新标记阶段的跨代引用复杂度‌。

        遍历年轻代存活对象，若其引用老年代对象，则将该老年代区域对应的卡表标记为脏页，后续重新标记阶段仅需扫描脏页区域‌。

    - 扫描脏卡

    经历过预清理阶段后，重新标记阶段对象可能有以下几种情况：

    - 如果某个存活对象被新生代重新引用，那么实际不需要以这个对象为根重新扫描，因为该对象本身就已经是存活的无需回收
    - 如果某个非存活对象被重新引用，那么需要以这个对象为根进行扫描，实际上这是在重新标记阶段需要完成的
    - 如果某个存活对象到新生代的引用关系被删除了，这个对象也被当作浮动垃圾，在本次的回收过程中无法被处理

    该阶段执行时长取决于许多因素，满足其中一个条件将会中止运行：
    ...
    - ‌ `-XX:CMSMaxAbortablePrecleanTime`
        控制可终止预清理阶段的最大执行时间（默认 5 秒），避免无限等待用户线程的 Minor GC‌。
    - `‌-XX:CMSScheduleRemarkEdenPenetration`
        设置 Eden 区使用率阈值（默认 50%），当 Eden 区占用超过该阈值时强制终止预清理，确保及时进入重新标记阶段‌。
...
2. CMS 收集器无法处理“浮动垃圾”（Floating Garbage）
    ...
    在 CMS 的并发标记和并发清理阶段，用户线程还在继续运行并伴随有新的垃圾对象不断产生，但这一部分垃圾对象是出现在标记过程结束以后，CMS 无法在当次收集中处理掉它们，只好留待下一次垃圾收集时再清理掉。这一部分垃圾就称为“浮动垃圾”。同样也是由于在垃圾收集阶段用户线程还需要持续运行，那就还需要预留足够内存空间提供给用户线程使用，因此 CMS 收集器不能像其他收集器那样等待到老年代几乎完全被填满了再进行收集，必须预留一部分空间供并发收集时的程序运作使用。
...
- 老年代内存占用达到阈值：老年代空间使用率超过 ‌ `-XX:CMSInitiatingOccupancyFraction` ‌ 参数设定值（默认 92%）时触发初始标记‌
- 方法区空间不足
- 年轻代晋升失败
- 显示调用垃圾回收
...
- 预防 Concurrent Mode Failure
     增大老年代空间或降低 `CMSInitiatingOccupancyFraction` 阈值‌，老年代空间使用率超过该值时触发初始标记。若需自定义阈值，需同时启用 ‌ `-XX:+UseCMSInitiatingOccupancyOnly` ‌ 参数，否则 JVM 可能动态调整触发条件‌。
- 优化回收耗时
    - 通过 `CMSScavengeBeforeRemark` 在 Remark 前触发 YGC，减少跨代引用跟踪负担
    - 增大 Survivor 区（`-XX:SurvivorRatio=6`），减少对象过早晋升至老年代
    - 调整大对象阈值 (`-XX:PretenureSizeThreshold`)：避免让一些中等大小、但生命周期不长的对象直接分配到老年代。合理设置这个阈值，让它们优先在新生代分配和回收，可以保持老年代的“干净”。
    - 设置最大停顿时间（`-XX:MaxGCPauseMillis=50`）‌‌‌
- 控制内存碎片
    - 启用 `UseCMSCompactAtFullCollection` 控制垃圾回收的同时进行内存碎片的压缩
    - 周期性 Full GC：`-XX:CMSFullGCsBeforeCompaction=4`  （例如每 4 次 Full GC 压缩一次内存碎片）‌‌
...
- 跨 Region 引用对象如何解决
    ...
    ![image.png](https://r2.129870.xyz/img/2025/1f26b84b485bdbb259cc3be041eaca81.png)

    在 G1 中，记忆集与卡表的关系如下：

    1. **卡表 (Card Table) 是一个具体的、全局的数据结构**
        它是一个字节数组，将整个 Java 堆划分为一个个固定大小（如 512B）的卡页（Card Page）。当一个 Region 中的对象引用了另一个 Region 中的对象时，JVM 会通过**写屏障 (Write Barrier)** 技术，将引用方对象所在的卡页标记为“脏”（Dirty），表示这里存在一个跨区引用。卡表本身并不知道引用的具体方向，只知道“这个卡页里有跨区引用”。
    2. **记忆集 (RSet) 用于解决跨区域引用问题**
        它的核心职责是解决跨区域引用的问题。对于任何一个 Region，它的 RSet 都记录了“有哪些其他 Region 中的对象引用了当前 Region 中的对象”。
        
        **RSet 存储的内容**：是一个哈希表，这个哈希表的 Key 是**引用了当前 Region 的其他 Region 的地址**。Value 则是一个**数组**，这个数组里存储的**不是**引用对象的直接地址，而是这些引用方对象所在**卡页在全局卡表中的索引（下标）**。
        
        **并发优化** (后台 Refinement 线程) ：G1 有专门的后台线程，称为并发优化线程（Concurrent Refinement Threads） 。这些线程会周期性地扫描全局卡表，寻找“脏卡”。当找到一个脏卡，线程会扫描这个卡片覆盖的 512 字节内存，找出其中所有指向外部 Region 的引用。于是，该线程会更新外部 Region A 的 RSet，在其中记下：“Region X 的第 N 号卡片引用了我”，更新完 RSet 后，这个脏卡状态会被清理，以便下次复用。
    
    1. JVM 通过**写屏障**更新全局的**卡表**，标记出哪些 512B 的内存块（卡页）存在跨区引用
    2. 每个 Region 的**RSet**（一个哈希表）会记录下：是哪个 Region（Key）的哪些卡页（Value，通过卡表索引表示）引用了自己。
    3. 当要回收某个 Region 时，GC 只需扫描其 RSet，就能快速定位到所有引用它的、精确到 512B 粒度的内存区域，而无需扫描整个堆
...
![G1收集器](https://r2.129870.xyz/img/2025/2012dda22045c5cbf7243e9ea34986c3.svg)

G1 收集器分为三种情况：

1. Young GC
    这是 G1 中最频繁发生的 GC 类型，也可以称之为 **Minor GC**。
    
    - 回收范围
        只回收所有 **Young Generation** 的 Region（即所有被标记为 Eden 和 Survivor 的 Region）。
    - 触发时机
        当 Eden Region 的数量达到预设的最大值，无法再为新对象分配空间时触发。
    - 执行方式
        1. 这是一个完全 **Stop-The-World (STW)** 的过程。
        2. 它采用**并行复制算法**。G1 会启动多个 GC 线程，将存活的对象从 Eden 和 Survivor Region 复制到新的 Survivor Region，或者如果对象年龄足够，则提升（Promote）到 Old Region。
        3. 这个过程非常快，因为新生代对象通常朝生夕死，存活对象很少。
    - 核心目标
        快速回收新生代中的垃圾，为新对象腾出空间。这与其他分代收集器中的 Minor GC 目标一致。

    Young GC 是 G1 的常规操作，其性能直接影响应用的吞吐量。它的 STW 时间非常短，通常在几十毫秒甚至几毫秒内，对应用影响很小。监控 Young GC 的频率和耗时是性能调优的基础。

    将 Young GC 单独拆分出去，是一个典型的**权衡（Trade-off）** 决策，它完美地平衡了**吞吐量（Throughput）** 和**延迟（Latency）** ：

    1. **最大化吞吐量**
        对于高频的新生代回收，使用算法最简单、CPU 占用最低的 STW 复制算法，将对应用线程的干扰降到最低。
    2. **保证低延迟**
        对于复杂、耗时的老年代回收，才启用重量级的并发标记技术，将 STW 控制在可预测的范围内，避免了传统的 Full GC 长时间停顿。
2. Mixed GC
    这是 G1 的精髓所在，也是它能够避免 Full GC 的关键。
    
    - 回收范围
        回收**所有的 Young Generation Region** 以及**部分 Old Generation Region**。注意，是“部分”而不是全部 Old Region，这是 Mixed GC 的核心特点。
    - 触发时机
        它不是由单一事件直接触发的，而是由一个复杂的**并发标记周期 (Concurrent Marking Cycle)** 驱动的。
        
        1. 当老年代的堆占用率达到某个阈值（由 `-XX:InitiatingHeapOccupancyPercent` 参数控制，默认 45%）时，G1 会启动并发标记周期。
        2. 这个周期包含多个阶段（如初始标记、并发标记、最终标记、筛选回收），大部分阶段是与应用程序**并发执行**的，不会导致长时间的 STW。
        3. 并发标记周期完成后，G1 就知道了哪些 Old Region 中包含了最多的垃圾（即“回收价值”最高）。
        4. 在此之后，G1 会安排执行一次或多次 Mixed GC。
    - 执行方式
        1. Mixed GC 本身也是 **Stop-The-World** 的，同样采用**并行复制算法**。
        2. G1 会根据用户设定的目标停顿时间（`-XX:MaxGCPauseMillis`），选择一组回收价值最高的 Old Region（称为 Collection Set, CSet），连同所有的 Young Region 一起进行回收。
        3. 通过分批次、有选择性地回收部分 Old Region，G1 将原本需要一次完成的、漫长的 Old GC 工作，化整为零，摊销到多次短暂停顿中。
    ...
    Mixed GC 是 G1 实现低延迟的关键。我们的目标就是**让系统稳定运行在 `Young GC -> 并发标记 -> Mixed GC` 这个良性循环中**。如果 Mixed GC 无法跟上垃圾产生的速度，最终就会导致问题。

3. Full GC
    这是 G1 的**后备方案**，是极力要避免的场景。一旦发生，意味着 G1 的低延迟承诺被打破。
    
    - 回收范围
        整堆回收，包括**所有**的 Young Region、Old Region，以及 Metaspace 等。
    - 触发时机
        
        1. 晋升失败 (Promotion Failure) / 空间分配失败 (Evacuation Failure) 
            在 Young GC 或 Mixed GC 期间，需要将存活对象复制到新的 Survivor 区或 Old 区，但找不到连续的可用空间来容纳这些对象。这通常是由于堆碎片化或者堆空间不足导致的。
        2. 并发模式失败
            在并发标记期间，如果应用程序创建对象的速度过快，导致堆在标记完成前就被耗尽，也会触发 Full GC。
        3. 显示调用
            代码中调用 `System.gc()`（除非通过 `-XX:+DisableExplicitGC` 禁用）。
        4. Humongous Object 分配失败
            分配一个巨大的对象时，在堆中找不到足够大的连续 Region。
    - 执行方式
        - 在早期的 JDK 版本中（如 JDK 8），G1 的 Full GC 是**单线程**的，采用标记-整理（Mark-Compact）算法，会导致非常长的 STW，性能极差。
        - 从 JDK 10 开始，G1 的 Full GC 优化为了**并行**模式，性能有所改善，但它仍然是一个非常耗时且应该避免的重量级操作。
...
- G1 的记忆集占用更多的内存空间
    ...
    相比起来 CMS 的卡表就相当简单，只有唯一一份，而且只需要处理老年代到新生代的引用，反过来则不需要，由于新生代的对象具有朝生夕灭的不稳定性，引用变化频繁，能省下这个区域的维护开销是很划算的。代价就是当 CMS 发生 Old GC 时（所有收集器中只有 CMS 有针对老年代的 Old GC），要**把整个新生代作为 GC Roots 来进行扫描**。
...
G1 调优的核心思想是：**设定一个明确的目标，然后给 G1 足够的灵活性去达成这个目标，而不是过度干预它的内部细节。**

 1. 核心目标参数 (首要配置)
    - `-Xms<size>` 和 `-Xmx<size>`
        
        - **作用**: 设置堆的初始大小和最大大小。
        - **建议**: 生产环境中**始终将两者设置为相同的值** (`-Xms` = `-Xmx`)。这可以避免堆在运行时动态伸缩带来的性能开销和不确定性。G1 在一个稳定的堆大小下工作得最好。
    - `-XX:MaxGCPauseMillis=<N>`
        
        - **作用**: 设置 G1 GC 的最大停顿时间的目标（一个软目标，单位毫秒）。
        - **建议**: 这是**G1 最重要的调优参数**。默认值是 `200` (ms)。你可以根据应用的延迟要求来设置，比如 `100` 或 `50`。G1 会尽力调整新生代大小、Mixed GC 的 CSet 等来满足这个目标。**不要设置得太低**（如低于 20ms），否则 G1 为了满足目标会使用非常小的新生代，导致频繁 GC，吞吐量下降。
2. 核心行为参数 (进阶调优)
    - `-XX:InitiatingHeapOccupancyPercent=<N>` (IHOP)
        
        - **作用**: 设置触发并发标记周期的老年代占用率阈值。默认值 `45` (即 45%)。
        - **建议**:
            - **如果频繁发生 Full GC**: 可能是因为并发标记启动得太晚，垃圾回收的速度跟不上对象晋升的速度。可以**适当调低这个值**（如 `35` - `40`），让 G1 更早地启动老年代回收。
            - **如果堆内存使用率很低但 GC 依然频繁**: 可能是该值设置得太低，导致不必要的并发标记周期，消耗 CPU 资源。可以适当调高。
    - `-XX:G1NewSizePercent=<N>` / `-XX:G1MaxNewSizePercent=<N>`
        
        - **作用**: 设置新生代占整个堆的最小/最大百分比。默认分别是 `5%` 和 `60%`。
        - **建议**: **通常不需要手动设置**。让 G1 根据 `-XX:MaxGCPauseMillis` 的目标自动调整新生代大小是最佳实践。只有在你明确知道新生代大小是瓶颈时才考虑调整，比如：
            - 如果 STW 时间主要消耗在对象复制上，可以考虑适当**减小** `-G1MaxNewSizePercent`。
            - 如果 Young GC 过于频繁，可以考虑适当**增大** `-G1NewSizePercent`。

3. 应对 Full GC 的专项参数

    当遇到因大对象（Humongous Object）导致的 Full GC 时，以下参数可能有用：

    - `-XX:G1HeapRegionSize=<N>`
        - **作用**: 手动设置 Region 的大小，必须是 2 的幂，范围从 1MB 到 32MB。默认 G1 会在启动时根据堆大小自动选择。一个对象如果大于 Region 大小的 50%，就会被认为是 Humongous Object。
        - **建议**: **一般不建议手动设置**。但如果应用中有大量大小中等（比如 2MB）的对象，而 G1 自动选择了 4MB 的 Region Size，导致大量 Humongous Allocation，你可以尝试手动将 Region Size 调大到 8MB，让这些对象能正常在 Eden 区分配。这是一种权衡，更大的 Region Size 可能会影响停顿时间。
...
1. 第一步：设定基础和目标
    ...
    - 设置 `-Xms` 和 `-Xmx` 为相同的值。
    - 设置一个合理的 `-XX:MaxGCPauseMillis` 作为你的延迟目标。
2. 第二步：开启 GC 日志并进行监控
    
    - 使用参数 `-Xlog:gc*:file=gc.log:time,level,tags:filecount=5,filesize=100m` (JDK 9+) 或 `-XX:+PrintGCDetails -XX:+PrintGCDateStamps -Xloggc:gc.log` (JDK 8) 开启详细的 GC 日志。
    - 使用 GCEasy、GCViewer 等工具分析日志，或使用 APM 工具（如 Prometheus + Grafana）进行实时监控。
3. 第三步：分析与迭代
    - 观察 P99/P95 停顿时间：是否满足 `-MaxGCPauseMillis` 的目标？
    - 观察 Full GC：是否出现了 `(Full GC)` 的日志？如果出现，这是最高优先级的警报。立刻检查 IHOP 设置、是否存在内存泄漏、或大对象分配问题。
    - 观察 Old Gen 占用率趋势：占用率是否能通过 Mixed GC 有效地降下来，形成一个稳定的波浪形？如果持续上涨，说明回收速度跟不上生产速度。
    - 基于数据进行调整：
        - Full GC -> 降低 IHOP。
        - 停顿时间超标 -> 检查 Young GC 和 Mixed GC 的 CSet 大小，G1 通常会自动调整，如果持续不达标可能意味着堆太小或应用分配模式有问题。
        - 吞吐量低 -> 适当放宽 `-MaxGCPauseMillis` ，给 G1 更多空间，减少 GC 频率。
...
- 染色指针可以作为一种可扩展的存储结构用来记录更多与对象标记、重定位过程相关的数据，以便日后进一步提高性能
    ...
    ![jvm地址空间.svg](https://r2.129870.xyz/img/2025/0112d7bed865a621ca17e139b57ea58f.svg)
...
1. 并发标记（Concurrent Mark）
    ...
    2. **并发标记阶段**
        ...
        CMS 和 G1 分别使用增量更新和原始快照的方式来解决并发标记的正确性，而 ZGC 则通过轻量级的读屏障来保证正确性。当应用程序线程加载到一个可能未被标记（白色）的引用时，**读屏障立即设置该引用的染色标记位（将其标记为灰色）**，并触发将该引用指向的对象加入标记处理流程。
...
1. 核心调优目标
    ZGC 调优的首要目标是**避免内存分配阻塞 (Allocation Stall)**。即确保垃圾回收的速度能跟上应用程序分配新对象的速度，防止因堆内存被占满而导致应用线程停顿。
2. 两大类调优场景及解决方案
    1. 内存分配阻塞，导致系统停顿（可达秒级）
        这类问题通常表现为 GC 日志中出现大量的 “Allocation Stall” 记录。
        
        * 问题场景一：流量突增（如秒杀活动）导致性能毛刺
            * **原因分析**：ZGC 默认的“基于分配速率的自适应算法”对于突发流量反应不够及时，可能在堆内存几乎耗尽时才触发 GC，导致线程阻塞。
            * **解决方法**：
                1. **设置固定时间间隔触发 GC**：使用 `-XX:ZCollectionInterval=<秒数>` 参数，强制 ZGC 定期执行，以应对流量洪峰。例如，设置为 `5` 秒。
                2. **提高自适应算法的敏感度**：增大 `-XX:ZAllocationSpikeTolerance=<系数>` 参数的值（默认为 2）。数值越大，ZGC 会越早地触发 GC。美团的实践中将其设置为 `5`。
        
        * 持续高压（如压测）导致性能毛刺
            * 原因分析：GC 触发很频繁，但回收速度跟不上分配速度，依然导致内存耗尽。
            * 解决方法
                **增加并发回收的线程数**：增大 `-XX:ConcGCThreads=<线程数>` 参数。这会分配更多的 CPU 资源给 GC 来加快标记和回收速度，但可能会对应用吞吐量产生一些影响。
    
    2. GC Roots 数量过大，导致单次 STW 停顿时间变长
        ...
        * CodeCache 过大
            * 原因分析：GC 日志中 “Pause Roots CodeCache” 耗时随运行时间增长。这是因为大量热点代码被 JIT 编译后存入 CodeCache，而 CodeCache 也属于 GC Roots。
            * 解决方法：这同样是**应用层面的优化**。通过业务优化，减少了进入 CodeCache 的方法数量。
...
| :--- | :--- |
| `-XX:+UseZGC` | 启用 ZGC。 |
| `-XX:ConcGCThreads` | 并发回收垃圾的线程数。调大可以加快 GC 速度，但会占用更多 CPU。 |
| `-XX:ZCollectionInterval` | 设置 GC 发生的固定时间间隔（秒），用于应对突发流量。 |
| `-XX:ZAllocationSpikeTolerance` | 自适应算法的修正系数（默认 2）。值越大，GC 触发越早、越频繁。 |
| `-XX:-ZProactive` | 关闭主动回收机制。如果已经使用了固定间隔触发，可以关闭此项以避免不必要的 GC。 |
| `-Xlog:gc*` | 记录详细的 GC 日志，是分析和调优的基础。 |

ZGC 的调优是一个结合了 **JVM 参数调整**和**应用程序代码优化**的综合过程。首先通过调整参数解决 GC 触发时机和回收效率问题，如果 STW 停顿依然较长，则需要深入分析 GC Roots，并从应用层面进行优化。
...
- 减少从主存读取数据的频率
    减少从主存读取数据频率的核心思路在于：如果 CPU 读取一个自己缓存没有的数据时，不是直接通过总线向主存读取，而是优先从其它已经缓存了对应数据的 CPU 缓存获取。
    ...
    > [!example]- 读取示例
    > 场景： CPU-A 需要读取一个内存地址 `0x1234` 的数据，但其 L1/L2 缓存均未命中（Cache Miss）。
    > 
    > 1. **L1/L2 缓存未命中**
    >     CPU-A 首先检查自己私有的 L1 和 L2 缓存，发现没有目标数据则再访问 L3 缓存。
    > 2. **访问共享 L3 缓存** 
    >     在大多数现代多核处理器中，会有一个所有核心共享的 L3 缓存（LLC, Last-Level Cache）。CPU-A 会先查询 L3 缓存：
    >     - **如果 L3 命中：** 直接从 L3 获取数据，速度很快，过程结束
    >     - **如果 L3 未命中：** 触发总线事务
    > 1. **发起总线读取事务**
    >     CPU-A 通过内存控制器向系统总线（System Bus）发起一个针对地址 `0x1234` 的 `“读请求”`。这个请求会被广播给系统中的**所有其他 CPU 核心**以及**主内存（RAM）** 。
    > 2. **总线嗅探（Snooping）** 
    >     其他所有的 CPU 核心（我们称之为 CPU-B, CPU-C...）内部都有一个“嗅探单元”，它会**持续监听**总线上的所有事务。当它们“嗅探”到 CPU-A 的读请求时，会立即检查自己的 L1/L2 缓存中是否持有地址 `0x1234` 的缓存行。
    > 3. **其他 CPU 的响应（关键分岔点）：**
    >     - **情况一：其他 CPU 缓存命中（Snoop Hit）**
    >         - 假设 CPU-B 的缓存中恰好有这份数据，并且状态为 **Modified (M)** 或 **Exclusive (E)** （意味着 CPU-B 持有最新、最权威的数据副本）
    >         - CPU-B 会立即响应总线，**中止**对主内存的访问请求（因为内存里的数据可能已经过时了）
    >         - CPU-B 将其缓存行中的数据直接通过总线发送给 CPU-A。这被称为 **Cache-to-Cache Transfer**
    >         - 同时，根据具体的协议（如 MESI），CPU-B 和 CPU-A 的缓存行状态会发生变更。例如，两者都可能变为 **Shared (S)** 状态。如果原先是 M 状态，CPU-B 可能还需要将数据写回主存
    >     - **情况二：所有 CPU 缓存均未命中**
    >         - 没有任何一个 CPU 核心响应这个读请求
    >         - 那么，最初发往主内存的请求会继续执行。数据将从主内存（RAM）中被读取，通过总线发送给 CPU-A
    >         - CPU-A 将数据加载到自己的缓存中，并将其状态标记为 **Exclusive (E)** ，表示目前只有它一份缓存副本
- 减少修改数据同步主存的频率
...
- I（Invalid）
    当前 CPU 中的缓存行数据无效，这往往是由于其他 CPU 对缓存行中的数据进行修改导致的，当前 CPU 如果去缓存中读数据的话，由于数据已经无效，会重新从内存中加载缓存行。
    
    > [!question] 为什么不是直接删除无效数据？
    > **标记为失效远比“删除”廉价、快速且更符合硬件的工作原理。**“缓存”在物理上是什么？它不是一个动态链表或哈希表，而是一个**固定大小的、静态的硬件阵列（SRAM）**基于硬件现实，采用一个专门的 I 状态带来了几个巨大的工程优势：
    > - 极致的极致的速度和效率
    >     操作成本极低： 将一个缓存行的状态从 S (Shared) 变为 I (Invalid) 只需要修改 2 个状态位（MESI 需要 2 bits 来表示 4 种状态）。这是一个微乎其微的电平翻转，在 1 个时钟周期内就能完成。
    > - 避免无效功耗
    >     如果要“删除”，可能需要清空整个 64 字节的数据块和几十位的 Tag 标签，这需要更多的时间和能量，产生不必要的热量。而标记为 I 状态则完全不需要动那些数据。旧数据留在那里，无伤大雅，直到下一个新数据需要这个位置时直接覆盖即可。
...
Store Buffer 位于每个 CPU 核心内部，逻辑上在执行单元和 L1 数据缓存之间。它不是缓存的一部分，而是一个独立的缓冲队列。每个核心都有自己私有的 Store Buffer。

它如何影响 L3 缓存的交互？Store Buffer 引入了延迟和异步：

- 写操作的延迟提交
    CPU-A 执行了写操作，但这个写操作可能还在 CPU-A 的 Store Buffer 里“排队”，尚未到达 L1 缓存，更不用说 L3 了。此时，从其他 CPU 核心或 L3 的视角来看，内存数据还没有被修改。
- 读操作的复杂化（Store Forwarding）
    如果 CPU-A 写入一个地址，然后紧接着又读取同一个地址，会发生什么？此时写入的数据还在 Store Buffer 里。为了保证程序的正确性，CPU 在执行读操作时，不仅要查询 L1/L2/L3 缓存，还必须首先检查自己的 Store Buffer。如果 Store Buffer 中有对应地址的条目，就直接从中获取数据，这个过程称为**写转发（Store Forwarding）** 。
- 对其他核心读请求的响应
    当 CPU-A 收到来自 CPU-B 的对某个地址的读请求时，情况变得更加复杂。CPU-A 的内存控制器不仅要检查自己的 L1/L2 缓存，还必须扫描自己的 Store Buffer。如果 Store Buffer 中存在针对该地址的、尚未提交的写操作，那么 Store Buffer 中的数据才是最新的。CPU-A 需要将此数据转发给 CPU-B，而不是 L1/L2 缓存中的（可能已过时的）数据。

所以，对 L3 交互的影响是： Store Buffer 的存在，使得一个核心的“真实内存状态”分成了两部分：一部分是已经体现在 L1/L2/L3 缓存层级中的状态，另一部分是“悬而未决”地暂存在 Store Buffer 中的最新修改。任何进出的内存请求都必须考虑这两部分，才能保证数据一致性。
