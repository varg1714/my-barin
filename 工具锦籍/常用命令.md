...
7. 编译安装
	...
	# 第一步后面的参数是为了配置https访问所需，如果需要支持http2，还需要安全其他模块。
...
4. 修改环境变量
	...
	set java environment JAVA_HOME=/usr/local/java/jdk1.8.0_261 JRE_HOME=/usr/local/java/jdk1.8.0_261/jre CLASS_PATH=.:$JAVA_HOME/lib/dt.jar:$JAVA_HOME/lib/tools.jar:$JRE_HOME/lib PATH=$PATH:$JAVA_HOME/bin:$JRE_HOME/bin export JAVA_HOME JRE_HOME CLASS_PATH PATH
