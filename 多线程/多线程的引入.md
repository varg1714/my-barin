...
Store Buﬀeres 是一个写的缓冲，对于上述描述的情况，CPU0 可以先把写入的操作先存储到 StoreBuﬀeres 中，Store Buﬀeres 中的指令再按照缓存一致性协议去发起其他 CPU 缓存行的失效。而同步来说 CPU0 可以不用等到 Acknowledgement，继续往下执行其他指令，直到收到 CPU0 收到 Acknowledgement 再更新到缓存，再从缓存同步到主内存。
