> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [juejin.cn](https://juejin.cn/post/6945260497864228871)

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d839150fb42d4dcfb227040bc34bf171~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/20df7653120b4811b2934d641f03d7bf~tplv-k3u1fbpfcp-zoom-1.image)
...
举个例子，google 的官网是 [www.google.com](http://www.google.com/) ，而 google 的 ip 地址是 216.58.200.228 ，这两个地址你在 URL 上输入哪个都能访问，但是 IP 地址不好记忆，而 google.com 简单明了。DNS 就相当于是我们几年前使用的家庭电话薄，比如你想给 cxuan 打电话，你有可能记不住 cxuan 的电话号码，此时你需要查询电话薄来找到 cxuan 的电话号码。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4e0e21c0538c43ad8721c2e516dafcd7~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/970e9c70a90448659cf2b4b7c95a2fad~tplv-k3u1fbpfcp-zoom-1.image)

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4bdbd92b667340d690cd590e62d043c4~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7e9d5a4feb02456ab3aa436bc5f51ec4~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e572437de47e45db9ac8b624b71e9059~tplv-k3u1fbpfcp-zoom-1.image)
...
所以对于 maps.google.com 这个域名来说，如果 ISP 维护的服务器没有 DNS 缓存记录，它就会向 DNS 根服务器地址发起查询，根名称服务器会将其重定向到 .com 顶级域名服务器。 .com 顶级域名服务器会将其重定向到 google.com 权威服务器。google.com 名称服务器将在其 DNS 记录中找到 maps.google.com 匹配的 IP 地址，并将其返回给您的 DNS 解析器，然后将其发送回你的浏览器。

这里值得注意的是，DNS 查询报文会经过许多路由器和设备才会达到根域名等服务器，每经过一个设备或者路由器都会使用`路由表` 来确定哪种路径是数据包达到目的地最快的选择。这里面涉及到路由选择算法，如果小伙伴们想要了解路由选择算法，可以看看这篇文章 [www.cisco.com/c/en/us/sup…](https://www.cisco.com/c/en/us/support/docs/ip/border-gateway-protocol-bgp/13753-25.html#anc3)
...
> 关于为什么有了 IP 地址，还要有 MAC 地址概述可以参看知乎这个回答 [www.zhihu.com/question/21…](https://www.zhihu.com/question/21546408)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e2a1ac04753146189742207c47d7efbb~tplv-k3u1fbpfcp-zoom-1.image)

主机 A 想要获取主机 B 的 MAC 地址，通过主机 A 会通过`广播` 的方式向以太网上的所有主机发送一个 `ARP 请求包`，这个 ARP 请求包中包含了主机 A 想要知道的主机 B 的 IP 地址的 MAC 地址。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2f63b7b82cf049a39ebf9915a10f4fef~tplv-k3u1fbpfcp-zoom-1.image)

主机 A 发送的 ARP 请求包会被同一链路上的所有主机 / 路由器接收并进行解析。每个主机 / 路由器都会检查 ARP 请求包中的信息，如果 ARP 请求包中的`目标 IP 地址` 和自己的相同，就会将自己主机的 MAC 地址写入响应包返回主机 A

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/655f42932b5a4f3da15c891d0e3edd1c~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f43f1ffe64784739863b732331a1c975~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9a2f3f2cbb6743319a49a13e0edebe5b~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1ec7b7dcaa794230adda2f97c1c4876a~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2690a7411ef34ae2910d55492ca8fb9f~tplv-k3u1fbpfcp-zoom-1.image)
...
（上述均来自：[github.com/skyline7548…](https://github.com/skyline75489/what-happens-when-zh_CN#dns%EF%BC%89)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/acd8f12743994f4192a77d3d4475c25f~tplv-k3u1fbpfcp-zoom-1.image)
...
在经过上述 DNS 和 ARP 查找流程后，浏览器就会收到一个目标服务器的 IP 和 MAC 地址，然后浏览器将会和目标服务器建立连接来传输信息。这里可以使用很多种 Internet 协议，但是 HTTP 协议建立连接所使用的运输层协议是 TCP 协议。所以这一步骤是浏览器与目标服务器建立 TCP 连接的过程。

TCP 的连接建立需要经过 TCP/IP 的三次握手，三次握手的过程其实就是浏览器和服务器交换 SYN 同步和 ACK 确认消息的过程。

假设图中左端是客户端主机，右端是服务端主机，一开始，两端都处于`CLOSED（关闭）`状态。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ce9d72f9c89a454eaa1b7d49262315be~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9b4f9a4f88c54aa699746871f73e0a7f~tplv-k3u1fbpfcp-zoom-1.image)

首先 Host 表示的是对象所在的主机。`Connection: close` 表示的是浏览器需要告诉服务器使用的是`非持久连接`。它要求服务器在发送完响应的对象后就关闭连接。`User-agent`: 这是请求头用来告诉 Web 服务器，浏览器使用的类型是 `Mozilla/5.0`，即 Firefox 浏览器。`Accept-language` 告诉 Web 服务器，浏览器想要得到对象的法语版本，前提是服务器需要支持法语类型，否则将会发送服务器的默认版本。下面我们针对主要的实体字段进行介绍（具体的可以参考 [developer.mozilla.org/zh-CN/docs/…](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers) MDN 官网学习）
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/95f42be270994cd5b45a3b447e8e5473~tplv-k3u1fbpfcp-zoom-1.image)

关于深入理解 HTTP 请求和响应，可以参考这篇文章

[看完这篇 HTTP，跟面试官扯皮就没问题了](https://mp.weixin.qq.com/s?__biz=MzkwMDE1MzkwNQ==&mid=2247496030&idx=1&sn=82f56874f82f372af71e23a8e385f8cd&chksm=c04ae600f73d6f16d707c1d32b00e3f0d47e893c9cf59a2eb60ace418943aeb5c5c679cb27ea&token=1307572136&lang=zh_CN#rd)
...
**我自己肝了六本 PDF，微信搜索「程序员 cxuan」关注公众号后，在后台回复 cxuan ，领取全部 PDF，这些 PDF 如下**

[六本 PDF 链接](https://s3.ax1x.com/2020/11/30/DgOK6f.png)

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/29d55231371f488299547ffc51d43561~tplv-k3u1fbpfcp-zoom-1.image)