...
1. 巧妙的再哈希设计
    - 位运算代替重新计算哈希值：通过一个巧妙的位操作判断键值对在新数组中的位置
    - 元素要么在原位置，要么在原位置加上旧容量
    - 原理：利用了 hash & oldCap 的结果，只需要判断扩容前 hashCode 对应的最高位是 0 还是 1
2. 链表转红黑树（JDK 1.8）
    - 当链表长度超过阈值（默认为 8）时，会转换为红黑树结构
    - 扩容时会优先考虑将红黑树拆分回链表（当红黑树节点数小于 6 时）
    - 提高了冲突严重情况下的查询效率
3. 扩容效率优化
    - 容量选择为 2 的幂：确保哈希值能均匀分布
    - 增量式扩容：非一次性完成所有元素迁移，降低单次扩容造成的性能抖动
    - 扩容因子默认 0.75：在时间和空间复杂度间取得良好平衡
4. 其他优化
    - JDK 1.8 中使用尾插法，避免了在多线程环境下可能出现的死循环问题
    - 优化了扩容触发条件，防止频繁扩容
    - 引入了更高效的哈希算法，减少碰撞概率
