...
2. 客户端获取锁的总耗时没有超过锁的有效时间。
    其目的是为了检测在**加锁期间**发生的长时间延迟或GC。如果客户端在拿到锁之前卡住了，这一步会检测出来并使加锁失败。如果客户端是在**成功拿到锁之后**才卡住，那么这个问题**对于任何分布式锁（包括ZooKeeper）都存在**，它已经超出了锁服务本身能解决的范畴。
...
而 Antirez 的认为：Redlock 并不要求时钟完全同步，而是允许一定误差。大的时钟跳跃被视为**运维问题**，一个稳定运行的系统应该通过恰当的运维（如平滑调整时间）来避免，这不应完全归咎于算法本身。
...
然而由于大多数资源本身不具备这种拒绝的能力，`fencing token` 机制要求被操作的**共享资源本身具备拒绝旧 token 的能力**。这对于数据库是可行的，但对于写文件、调用第三方 API 等场景则无能为力。因此 Antirez 认为，Redlock 加锁时设置的**唯一随机值 (unique_id)** 也可以起到类似 `fencing token` 的作用，通过在资源层做 CAS（Check-and-Set）操作来实现兜底。所以在实现上一种简化的设计思路是在 DB 层通过版本号的方式来更新数据，避免并发冲突。
...
- 在 `Redis 6.0` 中对网络模型实现了多线程 IO
	...
	![Mermaid Chart - Create complex, visual diagrams with text. A smarter way of creating diagrams.-2025-08-05-150227.svg](https://r2.129870.xyz/img/2025/6a2c72464a0e5d93a97e626aef58e310.svg)

	在这种设计下，IO 线程要么同时在读 socket，要么同时在写 socket，读写行为由主线程控制。为什么采取这种设计思路：

	1. 复杂性问题：内存管理、状态同步、错误处理的复杂度指数级增长
	2. 收益有限：真实瓶颈不在命令处理 CPU，而在网络和内存访问
	3. 风险过高：破坏 Redis 的可靠性和简单性优势
	4. 维护成本：bug 定位、性能调优、功能扩展都会变得极其困难

#### 3.2.2. 基于 Redis 分支的 Valkey

2024 年，Redis 商业支持公司 Redis Labs 宣布 Redis 核心代码的许可证从 BSD 变更为 RSALv2 ，明确禁止云厂商提供 Redis 托管服务，这一决定直接导致社区分裂。为维护开源自由，Linux 基金会联合多家科技公司（包括 AWS、Google Cloud、Oracle 等）宣布支持 [[Valkey 单点性能比肩 Redis 集群了？Valkey8_0 新特性分析｜得物技术|Valkey]] ，Valkey 基于 Redis 7.2.4 开发，作为 Redis 的替代分支。

Valkey 相较于 Redis 进行了多方面的改进：

![Untitled diagram _ Mermaid Chart-2025-08-09-135613.svg|300](https://r2.129870.xyz/img/2025/1d866bf9ba497deed48aa6429bf07c2b.svg)

1. 真正的异步线程模型    
    Redis 6.0 的局限性：
    - 同步等待模式：主线程阻塞等待所有 IO 线程完成
    - IO 线程空闲：主线程处理命令时，IO 线程处于空闲状态
    - 性能受限：受最慢 IO 线程速度限制
    - 功能单一：IO 线程仅处理读写和解析

    Valkey 8.0 的突破：

    - 异步任务队列：无锁环形缓冲区（大小 2048）
    - 真正异步：主线程发送任务后立即处理其他工作
    - 动态调整：根据负载动态调整活跃 IO 线程数量（最多 15 个）
    - 线程亲和性：同一客户端尽量由同一 IO 线程处理
2. 任务卸载扩展
    Valkey 8.0 将更多耗时操作从主线程卸载到 IO 线程：
    
    1. 事件轮询卸载
        优化前：主线程独自承担 epoll_wait 等系统调用
        优化后：IO 线程可并行执行套接字轮询，避免竞争条件
    2. 对象内存释放
        优化前：主线程处理完命令后还需释放参数对象内存
        优化后：同一 IO 线程负责分配和释放，保持内存局部性
    3. 命令查找预处理
        优化前：主线程执行命令时才查找命令字典
        优化后：IO 线程预先查找并缓存命令，主线程直接使用
3. 内存访问优化
    数据预取（Prefetch）技术：
    - 硬件指令：使用 `__builtin_prefetch()` 提前加载数据到 CPU 缓存（主内存 → L3 缓存 → L2 缓存 → L1 缓存）
    - 批量预取：对命令参数、key、value 进行批量预取
    - 非阻塞执行：预取指令立即返回，CPU 后台并行加载

    预取并不意味着 CPU 缓存能够全部容纳，但**即使部分命中也有收益**，且预取是根据 CPU 缓存策略，优先放入合适的缓存层级。因此只有任一缓存中能够命中，那么相较于从内存中读取都是有收益的。

    内存访问分摊（MAA）技术：

    - 交错执行：不再串行处理 key 查找，而是交错执行多个操作
    - 分阶段处理：
    ...
    - 延迟隐藏：在等待一个内存访问时处理其他操作
4. 架构设计优化
    线程通信机制：
    - 无锁设计：环形缓冲区避免锁开销
    - 原子标志：通过 `read_state` / `write_state` 原子标志通信
    - 单向通道：主线程到 IO 线程的高效消息传递

    批处理策略：

    - 命令批次：最多 16 条命令一批处理
    - 预取批次：批量预取数据后批量执行
    - 动态调整：根据**负载情况**动态优化批次大小

Valkey 的核心工作流程如下：

![Untitled diagram _ Mermaid Chart-2025-08-09-135426.svg](https://r2.129870.xyz/img/2025/efca598ef4a800852d6d909760632fa5.svg)
...
为了能高效地对经纬度进行比较，Redis 采用了业界广泛使用的 [[阅读中/文章列表/文章收藏/2025-09/GeoHash 算法学习讲解、解析及原理分析|GeoHash 编码方法]]，这个方法的基本原理就是 `二分区间，区间编码`。
...
> [!note] 不相邻方格的处理
> **本质上，GeoHash 的邻域计算不是在一个有限的二维数组中“寻找索引”，而是在一个连续且循环的空间中通过位操作来“计算”出邻居的编码。** 这种计算方式从数学上保证了无论一个点位于地球表面的任何位置——无论是赤道、本初子午线、国际日期变更线还是两极附近——它都能准确地计算出周围的所有 8 个邻居。
> 
> 因此， **“九宫格搜索”策略是普适且不存在漏洞的**，它能可靠地覆盖所有边界情况，确保不会遗漏任何物理上邻近的点。

![](https://r2.129870.xyz/img/20220518235015.png)

> [!info] 区域划分规则
> 我们将二进制编码的结果填写到空间中，当将空间划分为四块时候，编码的顺序分别是左下角 00，左上角 01，右下脚 10，右上角 11，也就是类似于 Z 的曲线，当我们递归的将各个块分解成更小的子块时，编码的顺序是自相似的（分形），每一个子快也形成 Z 曲线，这种类型的曲线被称为 Peano 空间填充曲线。
> 
> ![image.png](https://r2.129870.xyz/img/2025/2345a633d35d3d5e0f7ef6f017164807.png)
> 
> 空间填充曲线将二维的地理坐标（经纬度）映射到一维的字符串或整数上。想象一下，我们把地球表面不断地划分成越来越小的矩形网格。填充曲线的作用就是定义一种“遍历”所有这些小网格的顺序。

在计算出经纬度的Geohash后，它将 Geohash 转换为一个 **52位的整数**，这个整数就是 Geohash 二进制编码的十进制表示。然后它将这个 **Geohash 整数** 作为 `score` (分数)存入一个 ZSET 中。

当进行查询时：

1.  确定搜索范围
    Redis 根据给定的中心点和半径，计算出需要覆盖的 Geohash 网格。这包括中心网格和所有必要的**邻居网格**，它会自动处理边界问题。
2. 粗筛
    利用 ZSET 按照 `score` (也就是 Geohash 整数) 进行范围查找的能力（`ZRANGEBYSCORE`），极速地从 ZSET 中捞出所有可能在范围内的候选成员。
3. 精排
    Redis 遍历这些候选成员，逐一计算它们与中心点的**精确球面距离**（Haversine 公式）。
4. 过滤和排序
    它会剔除掉实际距离超出5公里的成员，并可以根据要求（例如 `WITHDIST` 和 `ASC` 选项）按距离从近到远排好序，最终返回结果。
...
1. 监控：监控主库运行状态，并判断主库是否客观下线
    ...
    2. 对于主库，若某个哨兵 ping 超时则将其标记为主观下线。需要多个哨兵都认为下线了才可将其标记成客观下线，此时主库真的会下线，开始执行选主流程。
...
1. `一致性 hash` 通常用来解决当集群节点数变化时，大量缓存无法命中的情况。因为 hash 计算方式为 $hashcode\div 节点数$，节点数的变化使得大部分计算结果都发生了变化。一致性 hash 可以保证同一个 key 在节点数变化的时候，大部分 $hashcode\div 节点数$ 仍然能计算出相同结果。
...
2. 根据映射信息请求哈希槽所在的实例。对请求到的实例可能出现以下几种情况：
    ...
    - 当前哈希槽正在迁移，返回 `ASK` 命令。客户端询问新实例该 key 的情况决定下一步操作。`ASK` 命令表示两层含义：
        ...
        `ASK` 机制确保了在迁移的中间状态下，对于一个给定的 key，它要么在源节点被处理，要么在目标节点被处理，绝不会出现两边都处理或两边都不处理的混乱情况。
        
        ![迁移并发写入.svg](https://r2.129870.xyz/img/2025/932ee9b86742fd2d53798ded36d6bbc4.svg)
...
Redis 的命令执行基于事件驱动机制，连接建立、命令解析执行、命令回复等都是基于不同的事件，整体运作流程如下：

![2025-08-10-145717.svg|300*300](https://r2.129870.xyz/img/2025/6199eaaa35d67230c5c5052e3f5ea55e.svg)
...
1. 建立连接
    ...
    2. ae 事件驱动模块接收
    3. 触发 `新建连接事件`
    4. acceptTcpHandler 函数执行 
    5. 创建 client 数据结构
2. 命令读取与解析
    当 Redis 收到命令后，会触发 ae 模块的 `读事件`，进入 readQueryFromClient 执行流程：
    
    - 单线程模式（未启用 IO 多线程）
        主线程独自执行：读取命令 → 解析命令 → 执行命令 → 发送结果
    - 多线程模式（启用 IO 多线程）
        1. 主线程将客户端加入 `clients_pending_read` 读就绪队列
        2. 客户端 flag 标记为 `CLIENT_PENDING_READ`
        3. 下次循环时分发给 IO 线程和主线程并行处理
        4. 最终由主线程执行命令

        命令解析步骤：

        5. 找到命令对应的执行函数，存放到 `client->cmd->proc`
        6. 解析参数，存放到 `client->argv`、`client->argc`

       ![Mermaid Chart - Create complex, visual diagrams with text. A smarter way of creating diagrams.-2025-08-10-151737.png](https://r2.129870.xyz/img/2025/08f27cbc708da133e0d5dc594322fc9d.png)

3. 命令执行
    - Redis 所有命令执行函数保存在 `redisCommandTable` 中
    - 例如 SET 命令对应 `setCommand` 函数
    - 执行完后调用 `addReply` → `prepareClientToWrite` 将结果加入 `clients_pending_write` 队列
...
4. 命令回复
    1. `beforeSleep` 函数将 `clients_pending_write` 分发给 IO 线程和主线程
    2. 通过 `writeToClient` 函数发送结果给客户端
    3. 如果一次无法发送完，则注册写事件，下次事件循环继续发送
    
    > [!question] 为什么需要下一次循环
    > 下一次循环不是延迟，而是 Redis 高性能设计的体现：
    > 
    > 1. **非阻塞原则**：Redis 主线程绝对不能被阻塞，当某个操作无法立即完成时，就推迟到下一次循环
    >     
    >     ![命令回复处理](https://r2.129870.xyz/img/2025/6bbe6303b0ca81d2e6e27e4074a24e33.svg)
    >     
    > 2. **批量优化**：收集多个操作一起处理，提高效率
    >     Redis 不是把数据收集到队列后立即处理，而是通过队列实现**批量处理**，例如将命令收集到 `clients_pending_read` 队列和 `clients_pending_write` 队列。
    >     
    >       为什么不立即处理？
    >       
    >       - 如果每个命令到达就立即处理，就是一个一个串行处理
    >       - 收集到队列后批量处理，可以并行处理多个命令，大大提高效率
...
INCR counter
DECR inventory
SADD processed_orders order:123
EXEC
```
...
- 事务失败时不支持回滚
> [!question] 为什么不支持回滚？
> Redis 采用性能有限原则，牺牲一致性来换取高可用性，因此其设计上避免了复杂的日志和回滚机制。
...
- 简化并发控制，避免多个客户端交错
- 批量操作优化
- 配合其他机制，例如与 Watch 和 lua 脚本协作
    Redis Watch 命令用于监视一个(或多个) key ，如果在事务执行之前这个(或这些) key 被其他命令所改动，那么事务将被打断。

#### 6.4.2 Redis 事务与管道的区别

- 命令处理机制层面
    - 管道（Pipeline） 采用的是**批量传输优化机制**
        当客户端使用管道时，多个命令被打包在一个或几个网络数据包中发送给 Redis 服务器。Redis 接收到这些命令后，会将它们依次放入命令执行队列中，然后按照先进先出的原则逐个执行。关键在于，这些命令**在 Redis 内部被视为独立的命令单元**，每个命令的执行结果都会被单独返回。
    
    - 事务（Transaction） 则实现了**命令组原子化机制**
        当 Redis 收到 MULTI 命令时，会为当前客户端创建一个事务上下文，后续的命令不会立即执行，而是被暂存在事务队列中。直到收到 EXEC 命令时，Redis 才会将整个事务队列中的所有命令作为一个不可分割的执行单元进行处理。
- 并发环境下的行为差异
    - 在多客户端并发场景下，**管道无法保证命令组的完整性**
        由于 Redis 的网络 I/O 处理机制，即使来自同一个管道的命令，也可能与其他客户端的命令交错进入执行队列。这种交错可能发生在网络传输层面（数据包分片、TCP 缓冲区处理）或应用层面（Redis 的命令解析过程）。
    - **事务通过 MULTI/EXEC 机制建立了执行屏障**
        在 MULTI 和 EXEC 之间的所有命令会被原子化执行，Redis 保证在处理一个事务的过程中，不会插入来自其他客户端的命令。这种机制确保了事务内命令的完整性和连续性。
- 数据一致性保证级别
    - **管道提供的是传输层面的优化**
        它解决的是网络往返次数的问题，但不提供任何数据一致性保证。如果业务逻辑要求多个操作必须保持同步状态，管道无法提供这种保障。
    - **事务提供的是逻辑层面的一致性保证**
        虽然 Redis 事务不支持回滚，但它确保了一组相关操作要么全部成功执行，要么在遇到语法错误时全部不执行。这种"全有或全无"的特性使得事务适合处理需要多步骤协调的业务场景。
- 执行时序和可见性
    管道中的命令执行时序取决于它们到达 Redis 命令队列的顺序，这个顺序可能受到网络状况、系统负载等因素影响。每个命令执行完毕后，其结果立即对其他客户端可见。事务中的命令执行时序是严格按照在事务中定义的顺序进行的，且所有命令的执行结果会在 EXEC 执行完成后同时对外界可见，呈现出原子性的状态变更。
- 错误处理机制
    - **管道的错误处理是独立的**
        某个命令出错不会影响其他命令的执行，每个命令的成功或失败状态都会在响应中独立反映。
    - **事务的错误处理分为两个阶段**
        在事务准备阶段（MULTI 之后，EXEC 之前）发现的语法错误会导致整个事务被丢弃；在事务执行阶段发现的运行时错误不会阻止其他命令的执行，但会在结果中标记出来。
 - 性能特征对比
    - 管道的性能优势主要体现在网络 I/O 优化上
        通过减少客户端与服务器之间的往返次数来提升整体吞吐量。由于没有额外的状态管理开销，管道是最轻量级的批量操作方式。
    - 事务的性能开销来源于状态维护和命令队列管理
        Redis 需要为每个事务维护独立的命令队列，并在 EXEC 时进行批量处理。虽然开销相对较小，但在极高频率的操作场景下，这种差异会变得明显。
- 适用场景边界
    - **管道适合纯粹的批量操作优化场景**
        特别是当多个操作之间没有强依赖关系，主要目标是提升网络传输效率时。典型应用包括批量数据导入、批量状态查询等。
    - **事务适合需要多步骤协调的业务逻辑场景**
        特别是当多个操作必须作为一个业务单元进行处理时。典型应用包括账户余额变更、库存扣减、状态同步更新等需要保持数据一致性的操作。
- 与 WATCH 命令的协作能力
    **管道无法与 WATCH 命令有效配合**，因为 WATCH 机制需要在事务上下文中才能发挥作用。**事务天然支持与 WATCH 命令的配合**，通过乐观锁机制可以实现更复杂的并发控制逻辑，这使得事务在处理有条件的原子操作时具备了更强的能力。

事务和管道解决的是不同层面的问题：管道解决的是**性能优化问题**，通过减少网络开销提升系统吞吐量；事务解决的是**逻辑正确性问题**，通过原子性执行保证数据操作的一致性。在设计 Redis 应用时，需要根据具体的业务需求和一致性要求来选择合适的机制，而不是简单地将两者视为功能相似的替代方案。
