...
集群划分好以后就涉及到客户端的使用了，那么客户端如何定位到 Redis 实例呢？这其实是一个[[数据密集系统设计/数据密集系统设计之复制与分区#2 4 请求路由|请求路由]]的问题，客户端通过与 Redis 集群建立连接后，可以知道 Redis 集群中所有哈希槽的映射关系，在操作数据使，按以下步骤进行：
...
- codis proxy：接收客户端请求，实现[[数据密集系统设计/数据密集系统设计之复制与分区#2 4 请求路由|请求路由]]把请求转发给 codis server。
