...
由于被驱动表无索引，因此是全表扫描。假设驱动表行数为 N，被驱动表为 M，则实际扫描行数就是 N\*M。这种直接扫描的做法我们称之为 `Simple Nested-Loop Join`。
...
- 这个参数的值被设置为 0 时，表示采用之前 MySQL 5.0 版本的策略，语句执行结束后才释放锁。
