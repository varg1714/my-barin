...
> [!warning] 连接池的复用问题
> 由于临时表是 session 结束时才会自动删除，而大多应用程序访问数据库是以连接池的形式进行的，考虑到连接池中的线程生命周期问题，使用临时表后应手动进行清除操作。
...
2. **增量数据同步**：
    ...
    **`gh-ost` 的核心设计是全量复制（Table Copy）和增量同步（Binlog Apply）并行进行**。这正是它实现“在线（Online）”变更、对主库影响小的关键。**那么，并行进行如何保证数据一致性？特别是“binlog 修改了一个不存在的数据”？**
    
    `gh-ost` 通过一套非常巧妙的机制来保证最终的数据一致性。
    
    1. 启动
        `gh-ost` 创建一个与原表结构相同但为空的“影子表”（`_tablename_gho`）。
    2. 监听 Binlog
        它开始从当前时间点监听主库上原表的 binlog 事件（`INSERT`, `UPDATE`, `DELETE`）。
    3. 全量复制
        同时，它开始从原表中读取数据行（通常是按主键顺序分块读取），并将其写入“影子表”。
    4. 并行应用 Binlog
        在全量复制的过程中，任何从 binlog 中捕获到的变更事件，都会被**立即应用**到“影子表”上。
    
    如果一个 `UPDATE` 或 `DELETE` 事件来了，但它对应的行还没有从原表复制到“影子表”中，会发生什么？
    
    1. **`gh-ost` 会照常在“影子表”上执行这个 `UPDATE` 或 `DELETE`。**
    2. 由于对应的行还不存在，这个操作会影响 `0` 行。**但这没有关系，也不会导致数据不一致。**
    3. **为什么呢？** 因为当稍后全量复制的进程进行到这一行时，它会从**原表**中读取这一行的**最新状态**（这个状态已经是被 `UPDATE` 修改过的了），然后将这个最新的数据写入“影子表”。
    4.  所以，即使早先的 `UPDATE` 事件“失败”了（影响 0 行），后续的全量复制操作会确保将正确的数据同步过来，覆盖了旧的状态（如果曾经存在的话）。
